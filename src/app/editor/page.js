...
import { faPlay, faCircleXmark, faExpand } from "@fortawesome/free-solid-svg-icons";
...
export default function Home() {
  ...
  function startCodePlayback() {
    ...
    playbackInterval = setInterval(() => {
      ...
      const currentEventIndex = events.findIndex(event => event.timestamp >= currentTime);
      ...
      if (currentEventIndex !== -1 && currentEventIndex !== lastEventIndex) {
        ...
        if (editorRef.current) {
          editorRef.current.getModel().setValue(currentEvent.files[fileName].value); // Update the editor value
        }
      }
  ...
  const toggleRecording = async () => {
    if (!recording) {
      ...
      recordingInterval = setInterval(() => {
        recordingDuration++;
        document.getElementById('status').textContent = `Recording... ${recordingDuration} seconds`;
      }, 1000);
      ...
      mediaRecorder.onstop = () => {
        ...
        document.getElementById('status').textContent = `Recording stopped after ${recordingDuration} seconds`;
      };
  ...
  function handleAudioSeek() {
    ...
    currentEventIndex = events.findIndex(event => event.timestamp >= seekTime);
  ...
  const handleEditorChange = (value, event) => {
    ...
    if (recording) {
      ...
      setEvents(prevEvents => [...prevEvents, { timestamp: currentTime, files: allFileContents }]);
    }
  }
    ...
    <>
      <div>
        <div className={styles.topBar}>
          ...
          <audio controls id="audioPlayback" onPlay={startCodePlayback} onSeeked={handleAudioSeek}></audio>
          ...
          <span id="status"></span>
          <button id="toggleData" onClick={toggleRecordedData}>Show Recorded Data</button>
        </div>
        <Editor
          height="70vh"
          theme="vs-dark"
      ...
      <Draggable>
        <div className={styles.websiteWindow} id="outputWindow">
          ...
          `}
            className={styles.outputiframewindow} id="outputIframe" />
        </div>
      </Draggable>
    </>
  );
